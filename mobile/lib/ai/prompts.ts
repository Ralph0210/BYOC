...
`
  ...
  const preset =
    PERSONALITY_PRESETS[
      (config?.personality_preset as keyof typeof PERSONALITY_PRESETS) ||
        "warm_encourager"
    ]
  ...
  if (Array.isArray(context) && context.length > 0) {
    const contextSummaries = context.map((item) => {
      if (item.type === "challenge") {
        ...
        const tasksList =
          item.tasks?.length > 0
            ? `\n- Tasks: ${item.tasks.map((t: any) => `"${t.name}" (${t.frequency_type}, ${t.frequency_count}x)`).join(", ")}`
            : ""
 ...
 */
export const buildMemoryExtractionPrompt = (existingMemories: any[] = []) => {
